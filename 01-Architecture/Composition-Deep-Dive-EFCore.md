# Composition vs Inheritance: Deep Dive with Value Objects & EF Core
...
1. **Clase Abstracta Person (Herencia de Datos):**
   * **Invariantes de Dominio:** Patient y Doctor no son tipos de persona en la lógica dental; son **ROLES y AGREGADOS independientes**.
   * **Impacto en BD (EF Core):**
     ...
     * **TPT (Table Per Type):** Genera múltiples tablas enlazadas por JOINs obligatorios, degradando la velocidad de lectura.

2. **Interfaz IPerson:**
   * Las interfaces definen **capacidades/contratos** (*can-do*), no estado persistible (*fields/properties*).
   * Crear IPerson cuando no existe un caso de uso polimórfico en la capa de aplicación es sobre-ingeniería (YAGNI).
...
* FirstName (
varchar(100)) <- *Proveniente de PersonName*
* LastName (
varchar(100)) <- *Proveniente de PersonName*
* Email (
varchar(150)) <- *Proveniente de ContactInfo*
* PhoneNumber (
varchar(20)) <- *Proveniente de ContactInfo*
* DateOfBirth (date)
